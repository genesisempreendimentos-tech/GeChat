...
export type GesiteDadosFilterOptions = {
  empreendimentos: string[];
  origens: string[];
  dispositivos: string[];
};

export const GESITE_BALANCO_OPTIONS: { value: GesiteBalancoMode; label: string }[] = [
...
export function defaultGesiteDadosFilters(): GesiteDadosFilters {
  return {
    ...defaultGesitePageControlFilters(),
    dataInicial: '',
    dataFinal: '',
...
export function collectGesiteDadosFilterOptions(rows: GesiteLeadMetricsRow[]): GesiteDadosFilterOptions {
...
export function filterGesiteDadosRows<T extends GesiteLeadMetricsRow>(
  rows: T[],
  filtros: Pick<GesiteDadosFilters, 'dataInicial' | 'dataFinal' | 'empreendimento' | 'origem' | 'dispositivo'>,
): T[] {
...
export function applyGesiteDadosPipeline<T extends GesiteLeadMetricsRow>(
  rows: T[],
  filtros: GesiteDadosFilters,
): T[] {
  const scoped = filterGesiteDadosRows(rows, filtros);
  const ranges = getGesiteBalancoRanges(filtros.balanco);
...
export function computeGesiteDadosBalanceCtx(
  filtros: GesiteDadosFilters,
  ...
  const base = filterGesiteDadosRows(rows, filtros);
