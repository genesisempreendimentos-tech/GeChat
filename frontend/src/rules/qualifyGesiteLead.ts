export type GesiteLeadQualificacao = 'Indefinida' | 'N/A' | 'Baixa' | 'Média' | 'Alta';
...
export function gesiteLeadRespondeuFormularioPerfil(row: LeadLike): boolean {
  return Boolean(
...
export function computeGesiteLeadQualificacao(row: LeadLike): GesiteLeadQualificacao {
  const hasContact = Boolean(row.email?.trim() || row.telefone?.trim() || row.contato?.trim());
  if (!hasContact) return 'N/A';

  if (!gesiteLeadRespondeuFormularioPerfil(row)) return 'Indefinida';
