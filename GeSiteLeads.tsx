...
const GESITE_LEADS_TAB_BUTTON_ITEMS = [
  { value: 'fonte' as const, label: 'Fonte', Icon: Globe },
...
function PontuacaoGaugeCard({ className, value = 72 }: { className?: string; value?: number }) {
  return (
    <div
      role="group"
      className={cn(
        'flex min-h-[16rem] flex-col gap-3 rounded-2xl border border-border/70 bg-card/80 p-5 shadow-sm backdrop-blur-sm dark:bg-card/60 lg:h-full',
        className,
      )}
    >
      <div className="flex shrink-0 items-center gap-1.5">
        ...
        <Tooltip delayDuration={200}>
          ...
          <TooltipContent
            ...
            collisionPadding={20}
            className={cn(
              'max-w-[min(13.333rem,calc((100vw_-_2rem)*2/3))] text-xs font-extralight leading-relaxed text-popover-foreground',
            )}
          >
...
export function computeGesiteLeadsBalanceCtx(balanco: GesitePageControlFilters['balanco']) {
  ...
  const countSac = (rows: GesiteLeadMockRow[]) => rows.filter((r) => r.pagina.toLowerCase().includes('sac')).length;
  const countTrab = (rows: GesiteLeadMockRow[]) =>
    rows.filter((r) => r.pagina.toLowerCase().includes('trabalhe')).length;
  const countForn = (rows: GesiteLeadMockRow[]) =>
    rows.filter((r) => r.pagina.toLowerCase().includes('fornecedor')).length;
  ...
  return {
    ...
    deltas: {
      ...
      ajudaSac: countSac(c) - countSac(p),
      trabalheConosco: countTrab(c) - countTrab(p),
      fornecedor: countForn(c) - countForn(p),
    },
  };
}
...
export const GeSiteLeads = forwardRef<GeSiteLeadsExportRef, GeSiteLeadsProps>(function GeSiteLeads(
  ...
  const [leadsPrimaryTab, setLeadsPrimaryTab] = useState<GesiteLeadsPrimaryTableTab>('paginas');
    ...
    <>
      {filtrosPanelAberto ? (
        ...
        />
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-[9fr_9fr_9fr_9fr_4fr] lg:grid-rows-2 lg:items-stretch">
                <InfoBox
                  title="Leads"
                ...
                />
                <PontuacaoGaugeCard
                  className={cn('min-h-[17rem] lg:min-h-0 lg:h-full', 'lg:col-start-5 lg:row-start-1 lg:row-span-2')}
                  value={88}
                />
                <InfoBox
                  title="Ajuda SAC"
                  value={GESITE_LEADS_INFOBOX_MOCK.ajudaSac}
                  balanceDelta={gesiteLeadsBalanceCtx?.deltas.ajudaSac}
                  balanceComparison={gesiteLeadsBalanceCtx?.comparison}
                  icon={<Hourglass />}
                  cor="muted"
                  infoTooltipAlign="end"
                  infoTooltip="Leads classificados com assunto Ajuda SAC."
                />
                <InfoBox
                  title="Trabalhe Conosco"
                  value={GESITE_LEADS_INFOBOX_MOCK.trabalheConosco}
                  balanceDelta={gesiteLeadsBalanceCtx?.deltas.trabalheConosco}
                  balanceComparison={gesiteLeadsBalanceCtx?.comparison}
                  icon={<Briefcase />}
                  cor="blue"
                  infoTooltipAlign="end"
                  infoTooltip="Leads classificados com assunto Trabalhe Conosco."
                />
                <InfoBox
                  title="Fornecedor"
                  value={GESITE_LEADS_INFOBOX_MOCK.fornecedor}
                  balanceDelta={gesiteLeadsBalanceCtx?.deltas.fornecedor}
                  balanceComparison={gesiteLeadsBalanceCtx?.comparison}
                  icon={<Hash />}
                  cor="violet"
                  infoTooltipAlign="start"
                  infoTooltip="Leads classificados com assunto Fornecedor."
                />
                <InfoBox
                  title="Principal Origem"
                  value={GESITE_LEADS_INFOBOX_MOCK.principalOrigem}
                  icon={<Eye />}
                  cor="emerald"
                  infoTooltipAlign="end"
                  infoTooltip="Origem com maior volume de leads no período."
                />
              </div>
              ...
              <div className="space-y-0">
                ...
                <div className="hidden md:block rounded-xl border border-border bg-muted/40 dark:bg-card overflow-x-auto">
                  {leadsPrimaryTab === 'paginas' ? (
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-border bg-muted/40 text-left text-xs text-muted-foreground">
                          <th className="p-3 font-medium whitespace-nowrap min-w-[120px]">
                            <button
                              type="button"
                      ...
                      <tbody>
                        {paginatedLeadsPaginasRows.map((row) => (
                          <tr key={row.id} className="border-b border-border/60 hover:bg-muted/20">
                            <th
                              scope="row"
                              className="p-3 text-left align-top text-sm font-medium text-foreground"
                            >
                              {row.nome}
                            </th>
                            {GESITE_LEADS_PAGINAS_COLS.map((col) => (
                      ...
                      <thead>
                        <tr className="border-b border-border bg-muted/40 text-left text-xs text-muted-foreground">
                          {leadsTableColumns.map((col) => {
                            ...
                            return (
                              <th key={col.key} className={cn('p-3 font-medium whitespace-nowrap', minWidthClass)}>
                                <button
                                  type="button"
      ...
      <Dialog
        ...
        >
          <DialogContent className="flex max-h-[min(90vh,48rem)] w-full flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
